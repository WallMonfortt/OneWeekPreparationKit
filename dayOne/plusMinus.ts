...
// the decimal value of each fraction on a new line with 6 digits places after the decimal.
...
function plusMinus (arr: number[]): void {
    const positive: number = arr.filter((item => item > 0 )).length;
    const negative: number = arr.filter((item => item < 0 )).length;
    const zero: number = arr.filter((item => item === 0 )).length;
